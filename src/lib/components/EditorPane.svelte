...
<div class="flex flex-col border-r border-slate-200 dark:border-slate-800">
  ...
  <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 px-6 py-2 bg-slate-100/30 border-b border-slate-200 dark:bg-slate-900/20 dark:border-slate-800 text-[11px] text-slate-500 dark:text-slate-400 font-medium" id="stats-bar-{side}">
    <span class="flex items-center gap-1 bg-slate-100/60 dark:bg-slate-800/40 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300">
      <span class="font-semibold text-slate-800 dark:text-slate-200">{stats.lines}</span> lines
    </span>
    <span class="flex items-center gap-1 bg-slate-100/60 dark:bg-slate-800/40 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300" id="{side}-stat-words">
    ...
    <span class="flex items-center gap-1 bg-slate-100/60 dark:bg-slate-800/40 px-1.5 py-0.5 rounded text-slate-600 dark:text-slate-300" title="Expected reading duration at an average rate of 200 words per minute">
  ...
  <div class="flex flex-wrap items-center justify-between gap-2 px-4 py-1.5 bg-slate-50 border-b border-slate-200 dark:bg-slate-900 dark:border-slate-800" id="{side}-text-operations-bar">
    <div class="flex flex-wrap items-center gap-1.5">
      <button
        ...
        class="px-2 py-1 text-[11px] font-medium text-slate-600 dark:text-slate-300 bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700 rounded transition shadow-2xs"
      ...
      <div class="relative inline-block text-left">
        <button
          ...
          class="px-2 py-1 text-[11px] font-medium rounded border transition shadow-2xs {showReplace ? 'bg-indigo-50 border-indigo-200 text-indigo-700 dark:bg-indigo-950 dark:border-indigo-900 dark:text-indigo-300' : 'text-slate-600 dark:text-slate-300 bg-white hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'}"
  ...
  <div class="flex-1 min-h-[220px] relative">
    ...
    {#if value === ''}
      <div class="absolute bottom-6 right-6 flex items-center gap-2 pointer-events-none select-none">
        <span class="text-[11px] font-mono text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-950/40 px-2 py-1 rounded">
          {emptyHint}
        </span>
      </div>
    {/if}
